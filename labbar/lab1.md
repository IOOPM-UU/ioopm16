...
heltalet 2016. (Minns att den typ är ett namn på en samling värden
...
`let x = 5 in BLAHRG`. Nu är `x` en konstant vars värde är `5`.
När vi har denna typ av namnbindningar kan vi söka och ersatta
alla förekomster av `x` med `5` i `BLAHRG`. 

Variabler i C funkerar annorlunda i och med att deras värden kan
...
ändra värde. Om `...` ovan t.ex. är 
    ...
    print_int(x); // en påhittad funktion som skriver ut en int
    x = 42;
    print_int(x)
...
Använd en **for-loop** och utgå från följande programskelett:
...
```c
for ( deklaration av iterationsvariabel ; 
      iterationsvillkor ; 
...
skriv om programmet så att istället för `1` så skriv en `#` ut,
och så vidare. Sist skall det skrivas ut hur många `#`
...
Skriva ut _N_ `#` kan enkelt göras med en loop som i _N_ varv
skriver ut ett `#` i varje varv utan efterföljande radbrytning. 
...
Kommandoradsargument skickas automagiskt in som argument till `main()`-funktionen. 
Vi kan skriva programmet `eko` så här (som bara "ekar" alla kommandoradsargument): 
...
Minns att **en array** är som en lista -- dvs. det är en samling
av värden som inte kan utökas. Om arrayen `arr` är en array med
tre element kan vi komma åt element 1 med `arr[0]`, element 2 med
`arr[1]` och element 3 med `arr[2]`. Om vi skriver `arr[3]` för att
komma åt det 4:e elementet är resultatet skräp, och exakt vad det
betyder kan variera mellan olika implementationer av kompilatorer,
etc. Det är alltså något som vi skall undvika, men som kompilatorn
inte skyddar oss emot. 

Arrayer indexeras från 0, dvs. `puts(argv[0])` skriver ut den första 
strängen i `argv`. `arv[0] = "Ivor Cutler"` gör så att den första
strängen i `argv`-arrayen är strängen "Ivor Cutler". 
...
Den märkiga `||`-symbolen är ett logisk _eller_ dvs. `a || b` är
...
_y_ i intervallet [2.._N_). Om det inte går att hitta ett sådant 
...
nybörjade som noviser -- ingen tycker att det är en bra idé att
...
att inspektera varje tecken i strängen är en siffa (0-9). 

* Funktionen `strlen()` i bilioteket `string.h` (`#include <string.h>`) returnerar längden av en sträng (med eller utan nulltecknet? Utforska!)
* Funktionen `isdigit()` i biblioteket `ctype.h` testar om ett enskilt tecken är en siffra
* Implementera testet med hjäpl av en loop från `i=0` till längden av strängen (utan nulltecknet)
...
fått det att fungera, kopiera funktionen (och importer-satser)
över in i `p6.c` och använd funktionen för att kontrollera att
kommandoradsargumenten är heltal. Lägg också in en kontroll att
de inte är negativa (dvs. `talet >= 0`). 
...
radordning, och om den inte först har sett en funktions **deklaration**
så kan den inte kontrollera att anropen till den är korrekta, t.ex.
map. antalet parametrar. Redan i nästa lab skall vi se hur man kan
komma runt detta med så-kallade `header`-filer.
...
flera `return`-satser utan att det kan bli tveksamt vad funktionen
...
det inskickade tecknet `c` är `'0' <= c && c <= '9'`. Glöm inte
att ta bort importen av `ctype.h` eller döpa din funktion (t.ex. till
`is_digit()`) så att namnet inte krockar med `ctype.h`:s funktion.
...
I modern C använder vi ofta datatyper vars storlek är garaneterad
